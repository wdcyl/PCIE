...
本工程面向ALINX AX7103（Artix-7 XC7A100T）与AN9238采集模块。FPGA完成双路12-bit、65 MSPS采样，将数据写入板载缓存；主机通过Xilinx XDMA的C2H通道读取采样区，并完成保存、校验、速率统计和波形显示。
...
```text
模拟输入 -> AN9238 -> 采样/128-bit打包 -> AXI突发写入 -> 板载存储
                                                            |
...
sudo software/pcie_capture --mode adc --bytes 1048576 --out adc.bin
python software/plot_capture.py adc.bin --count 4096
```

详细步骤见[设计说明](docs/DESIGN.md)、[Vivado与IP配置](docs/VIVADO_BUILD.md)、[上板流程](docs/HARDWARE_BRINGUP.md)、[寄存器表](docs/REGISTER_MAP.md)和[验证记录](docs/VERIFICATION.md)。
